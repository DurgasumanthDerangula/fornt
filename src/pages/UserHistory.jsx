  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const historyRes = await axios.get(`https://healthback-1-rx2e.onrender.com/api/scores/history/${userId}`);
        setScores(historyRes.data);

        // Get user details to fetch the name
        const userRes = await axios.get(`https://healthback-1-rx2e.onrender.com/api/auth/users`);
